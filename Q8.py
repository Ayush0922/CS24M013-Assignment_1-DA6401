...
def train(config=None):
    with wandb.init(config=config):
        ...
        while epoch < config.epochs:
            ...
            train_loss = np.mean(np.sum((y_train_pred - y_train) ** 2, axis=1))  # MSE loss
            train_accuracy = np.mean(np.argmax(y_train_pred, axis=1) == np.argmax(y_train, axis=1))

            # Compute validation loss and accuracy
            y_val_pred = model.forward(X_val)
            val_loss = np.mean(np.sum((y_val_pred - y_val) ** 2, axis=1))  # MSE loss
            val_accuracy = np.mean(np.argmax(y_val_pred, axis=1) == np.argmax(y_val, axis=1))

            # Log metrics
            wandb.log({
                "MSE_epoch_2": epoch,
                "MSE_train_loss_2": train_loss,
                "MSE_train_accuracy_2": train_accuracy,
                "MSE_val_loss_2": val_loss,
                "MSE_val_accuracy_2": val_accuracy
            })
            epoch += 1
